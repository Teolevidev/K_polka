<!--
  ВНЕШНИЙ РЕФЕРЕНС. Это style-документ чужого продукта (literal.club),
  сохранен дословно как пример. Дизайн-системой K_polka он не является -
  у нас будет свой дизайн. Разбор по существу: literal-analysis.md.
  Текст ниже не редактируем, чтобы он оставался точной копией источника.
-->
