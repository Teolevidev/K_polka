 ...
 * известных книг. Обложку и страницу книги ищем по ISBN: он у витрины
 * есть, а идентификатора тома в каталоге - нет.
 *
...
function toCard(b: ShowcaseBook): BookCardData {
  return {
    ...
    // Ведем на саму книгу, а не в поиск по названию: книга в витрине
    // уже выбрана, и показывать вместо нее список из двадцати изданий
    // нелогично. Идентификатора тома у витрины нет, поэтому переход
    // идет по ISBN через маршрут-посредник.
    //
    // Название и автора передаем туда же: ISBN здесь конкретного
    // издания, и каталоги знают далеко не каждое - без запасного пути
    // переход упирался в пустую выдачу.
    href:
      `/book/isbn/${b.isbn}` +
      `?t=${encodeURIComponent(b.title)}&a=${encodeURIComponent(b.author)}`,
    rating: b.rating,
  };
}
